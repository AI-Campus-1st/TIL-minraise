...
SELECT *
FROM tb_customer
-- WHERE grade != 'GOLD';  -- NULL 값이 빠지므로 주의
...
SELECT *
FROM tb_order
-- SELECT DISTINCT status from tb_order;
...
-- 4. 등급 없는 고객을 '미지정'으로
SELECT customer_name,
    IFNULL(grade, '미지정') AS grade -- 칼럼 이름을 AS로 바꿔준다.
FROM tb_customer;
-- AS = alias
...
-- 실습 2-4)
-- Q.연도를 행으로, 1~6월을 열로 하여 월별 신규 고객수 조회
-- tb_customer
...
-- 실습 3-7) HAVING
-- Q. 국가별로 고객수가 2명 초과인 국가만 조회(고객수 기준 내림차순)
-- tb_customer
SELECT country, COUNT(*) AS 고객수
FROM tb_customer
GROUP BY country
...
-- Q. 상품수가 3개보다 많은 카테고리 조회(상품수 기준 내림차순)
-- tb_product
...
-- 실습 4-2)
-- Q. 상품의 이름, 가격, 전체 평균 가격, 가격과 전체 평균 가격의 차이 조회
-- tb_product
SELECT AVG(unit_price)
FROM tb_product;
...
-- 실습 4-3)
-- Q. 월별 주문건수를 구한 뒤, 주문건수가 7건 이상인 달 조회
-- tb_order
...
-- 주문 테이블에서 고객의 id를 하나씩 확인해서 있으면 1 조회(주문개수만큼)
...
-- 03 카테고리별로 상품 가격의 합, 가장 비싼 상품 가격, 
-- 가장 저렴한 상품가격을 구하세요.
...
-- 04 카테고리별로 총재고가 300개가 넘을 경우 많음, 아니면 적음이 표시된
-- 칼럼을 추가하고, 재고수 많은 순서대로 조회.
SELECT category_id,
...
-- 05 국가별 고객수와 백분위(국가별 고객수/전체 고객수*100)를 구하세요.
-- 전체 고객수 먼저(서브 쿼리)
SELECT COUNT(*)
FROM tb_customer;
...
-- 실습 6-1) 다중조인
-- Q. 취소되지 않은 주문의 고객명, 상품명, 품목 수량, 주문금액 조회
...
-- FROM tb_order AS o
--     JOIN tb_customer AS c ON o.customer_id=c.customer_id
--     JOIN tb_order_item AS oi ON oi.order_id=o.orer_id;

-- Q. 카테고리별 취소되지 않은 주문건수·수량·매출을 매출 높은 순으로 조회
...
-- Q. 취소되지 않은 주문의 고객별 구매액 TOP 5을 조회
